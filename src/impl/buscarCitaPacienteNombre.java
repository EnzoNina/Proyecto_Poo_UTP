...
public class buscarCitaPacienteNombre implements IbuscarCitaPor{
    ...
    public DefaultTableModel buscarCitaPor(buscarCita objetoBuscar) {
        ...
        objetoBuscar.getArray_cita().stream().filter(array_cita -> (array_cita.getPaciente().getNombre().equalsIgnoreCase(objetoBuscar.getBuscarTexto()))).forEachOrdered(array_cita -> {
