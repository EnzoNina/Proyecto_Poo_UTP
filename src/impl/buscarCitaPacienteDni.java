...
public class buscarCitaPacienteDni implements IbuscarCitaPor{
    ...
    public DefaultTableModel buscarCitaPor(buscarCita objetoBuscar) {
        ...
        objetoBuscar.getArray_cita().stream().filter(array_cita -> (array_cita.getPaciente().getDNI().equalsIgnoreCase(objetoBuscar.getBuscarTexto()))).forEachOrdered(array_cita -> {
