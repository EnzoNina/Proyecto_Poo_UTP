...
public class buscarPorApellido implements IbuscarPor{
    @Override
    public DefaultTableModel buscarPor(Buscar objetoBuscar) {
        ...
        objetoBuscar.getArray_persona().stream().filter(persona -> (persona instanceof Doctor)).filter(persona -> (((Doctor) persona).getApellido().equalsIgnoreCase(objetoBuscar.getBuscarTexto()))).forEachOrdered(persona -> {
