...
public class buscarPacientePorNombre implements IbuscarPor {
    ...
    public DefaultTableModel buscarPor(Buscar objetoBuscar) {
        ...
        objetoBuscar.getArray_persona().stream().filter(persona -> (persona instanceof Paciente)).filter(persona -> (((Paciente) persona).getNombre().equalsIgnoreCase(objetoBuscar.getBuscarTexto()))).forEachOrdered(persona -> {
