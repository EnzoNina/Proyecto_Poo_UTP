...
public class buscarCitaPacienteApellido implements IbuscarCitaPor{
    ...
    public DefaultTableModel buscarCitaPor(buscarCita objetoBuscar) {
        ...
        objetoBuscar.getArray_cita().stream().filter(array_cita -> (array_cita.getPaciente().getApellido().equalsIgnoreCase(objetoBuscar.getBuscarTexto()))).forEachOrdered(array_cita -> {
