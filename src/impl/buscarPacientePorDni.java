...
public class buscarPacientePorDni implements IbuscarPor{
    @Override
    public DefaultTableModel buscarPor(Buscar objetoBuscar) {
        ...
        objetoBuscar.getArray_persona().stream().filter(persona -> (persona instanceof Paciente)).filter(persona -> (((Paciente) persona).getDNI().equalsIgnoreCase(objetoBuscar.getBuscarTexto()))).forEachOrdered(persona -> {
