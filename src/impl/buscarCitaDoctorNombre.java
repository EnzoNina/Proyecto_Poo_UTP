...
public class buscarCitaDoctorNombre implements IbuscarCitaPor{
    ...
    public DefaultTableModel buscarCitaPor(buscarCita objetoBuscar) {
        ...
        objetoBuscar.getArray_cita().stream().filter(array_cita -> (array_cita.getDoctor().getNombre().equalsIgnoreCase(objetoBuscar.getBuscarTexto()))).forEachOrdered(array_cita -> {
        ...
        }); //agarra la tabla de citas y las trcottr
